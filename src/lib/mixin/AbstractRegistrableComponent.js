...
import { COMPONENT_ID, IS_READY } from '../enum/Functions.ts';

// Tree shaking not working with current webpack setup!
...
export default {
  ...
  methods: {
    ...
    hasChild(componentId, componentType) {
      ...
      const requestedChild = this.$children.find(child => child[COMPONENT_ID] === componentId);
      // Check if if the child exists and/or if the type is correct
      return (
        requestedChild !== undefined &&
        (componentType === undefined || requestedChild.componentType === componentType)
      );
    },
    /**
     * @public
    ...
    getChild(componentId, componentType) {
      ...
      if (requestedChild) {
        if (componentType !== undefined) {
          if (requestedChild.componentType === componentType) {
      ...
      throw new Error(
        `Requested component [${componentId}] is not found within [${this.componentId}]`,
      );
    },
    /**
     * @public
    ...
    $_updateRegistrableComponents() {
      this.registrableComponents = this.$children.filter(
        childComponent => childComponent[COMPONENT_ID] !== undefined,
      );
    },
  },
  mounted() {
